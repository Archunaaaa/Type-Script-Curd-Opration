      ...
      try {
        if (isEditing && editItemId !== false) {
          ...
          if (response.ok) {
            ...
            setItems((prevItems) =>
              prevItems.map((item) => (item.id === editsItemId ? updatedItem : item))
            );
...
const handeltable=()=>{
  ...
  return (
    <div>
      <button className='rounded mt-5 mb-3 ms-5' onClick={handeltable}>Go To Table
        
        </button>
        <div className='container w-75'>
      <div className='width '>
      <div className=' mt-5'>
        <h1 className='ms- mt-3 text-center text-success'>Crud operation</h1>
        </div>
  ...
  <input
    ...
    onChange={(e) => {
      ...
      setValidationErrors((prevErrors) => ({
        ...prevErrors,
        name: e.target.value.trim().length < 3 ? 'Name must be at least 3 letters' : '',
      }));
    }}
  />
...
<div className="col-md-6">
  ...
  <input
    ...
    onChange={(e) => {
      setNewItem((prev) => ({ ...prev, phoneNumber: e.target.value }));
      setValidationErrors((prevErrors) => ({
        ...prevErrors,
  ...
  {validationErrors.phoneNumber && (
    <p className="error text-danger">{validationErrors.phoneNumber}</p>
  )}
</div>
...
<div className="col-md-6">
  ...
  type="password"
    ...
    onChange={(e) => {
      ...
      setValidationErrors((prevErrors) => ({
        ...prevErrors,
        password: e.target.value.trim() === '' ? 'Password is required' : '',
      }));
    }}
  />
...
<div className="col-md-6">
  ...
  <input
    ...
    onChange={(e) => {
      setNewItem((prev) => ({ ...prev, confirmedPassword: e.target.value }));
      setValidationErrors((prevErrors) => ({
        ...prevErrors,
        confirmedPassword: e.target.value.trim() === '' ? 'Confirm Password is required' : '',
      }));
    }}
  />
  {validationErrors.confirmedPassword && (
    <p className="error text-danger">{validationErrors.confirmedPassword}</p>
  )}
</div>
        </div>
...
<div className="col-md-6">
  ...
  <select
    ...
    onChange={(e) => {
      ...
      setValidationErrors((prevErrors) => ({
        ...prevErrors,
        gender: e.target.value.trim() === '' ? 'Gender is required' : '',
      }));
    }}
 >
...
<div className="col-md-6">
 ...
 <div>
      ...
      {/* <Table items={items} onEdit={handleEditItem} onDelete={handleDeleteItem} /> */}
    </div>
    </div>
  );
};
